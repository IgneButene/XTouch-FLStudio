# name=Behringer X-Touch Extender v2
...
class TMackieCU_Ext(mcu_base_class.McuBaseClass):
    def __init__(self):
        device = mcu_device.McuDevice(True)
        ...
        if (event.midiId == midi.MIDI_CONTROLCHANGE):
            if (event.midiChan == 0):
                ...
                if event.data1 in [mcu_knobs.Knob_1, mcu_knobs.Knob_2, mcu_knobs.Knob_3, mcu_knobs.Knob_4, mcu_knobs.Knob_5, mcu_knobs.Knob_6, mcu_knobs.Knob_7, mcu_knobs.Knob_8]:
                    ...
                    if self.Page == mcu_pages.Instruments:
                        i = event.data1 - mcu_knobs.Knob_1
                        ...
                        self.McuDeviceMessages.SendMsg('Free knob ' + str(event.data1) + ': ' + s + str(abs(event.outEv)))
            ...
            if event.midiChan <= 8:
                ...
                if self.Page == mcu_pages.Instruments:
                    ...
                    event.midiChan = 0
                    event.midiChanEx = 0
                    self.McuDeviceMessages.SendMsg('Free slider ' + str(event.data1) + ': ' + ui.getHintValue(event.outEv, 65523))
                    ...
                    if s != '':
                        s = ': ' + s
                    self.McuDeviceMessages.SendMsg(self.Tracks[event.midiChan].SliderName + s)
            ...
            if event.midiId == midi.MIDI_NOTEON:
                ...
                if (event.pmeFlags & midi.PME_System != 0):
                    if event.data1 == mcu_buttons.NameValue: # display mode
                        if event.data2 > 0:
                            ...
                            #self.McuDeviceMessages.SendMsg(self.MackieCU_MeterModeNameT[self.MeterMode])
                            #self.UpdateMeterMode()
                        ...
                        if self.Page == mcu_pages.Instruments:
                            ...
                            if event.data2 > 0:
                                ...
                                event.outEv = 0
                                event.isIncrement = 2
                                self.McuDeviceMessages.SendMsg('Free knob switch ' + str(event.data1))
                                device.processMIDICC(event)
                            device.hardwareRefreshMixerTrack(self.Tracks[i].TrackNum)
                            return
                        elif event.data2 > 0:
                        ...
                        if self.Page == mcu_pages.Instruments:
                            ...
                            self.McuDeviceMessages.SendMsg('Free button ' + str(event.data1) + ': ' + mcu_constants.OffOnStr[event.outEv > 0])
                            device.processMIDICC(event)
                            device.hardwareRefreshMixerTrack(self.Tracks[i].TrackNum)
                            return
                ...
                if (event.pmeFlags & midi.PME_System_Safe != 0):
                    if event.data1 == mcu_buttons.Option: # link selected channels to current mixer track
                        if event.data2 > 0:
                            if self.Shift:
